 ...
 */
function createSequentialSink(handler, errorHandler, options = { timeout: Infinity, timeoutHandler: () => { } }) {
    const q = new queue_js_1.DecayingDeque(options.timeout, handler.consume, false, errorHandler, options.timeoutHandler);
    return {
 ...
 */
function createBatchSink(handler, errorHandler, options = { timeout: Infinity, timeoutHandler: () => { } }) {
    const q = new queue_js_1.DecayingDeque(options.timeout, handler.consume, false, errorHandler, options.timeoutHandler);
 ...
 */
function createConcurrentSink(handler, errorHandler, concurrency = 500, options = { timeout: Infinity, timeoutHandler: () => { } }) {
    const q = new queue_js_1.DecayingDeque(options.timeout, handler.consume, concurrency, errorHandler, options.timeoutHandler);
    return {
